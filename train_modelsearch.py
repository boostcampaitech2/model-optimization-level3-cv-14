...
def objective(trial: optuna.trial.Trial, device) -> Tuple[float, int, float]:
    ...
    with open('./exp/latest/data.yml') as f:
        data_config = yaml.load(f,yaml.SafeLoader)
    ...
    ###### model #######
    with open('./exp/latest/model.yml') as f:
    ...
    trainer = TorchTrainer(
        ...
        optimizer=optimizer,
        scheduler=scheduler,
        model_path='./exp/latest/best.pt', #TODO:args로 받기
        scaler=None, #??
        device=device,
        verbose=1
    )
...
def main(gpu_id,storage:str=None):
    ...
    with open('./exp/latest/config.txt','w') as f:
        f.write(best_trial)
