...
class TorchTrainer:
    ...
    def train(
        ...
        for epoch in range(n_epoch):
            ...
            for batch, (data, labels) in pbar:
                ...
                wandb.log({
                    'train/trial':trial.number,
                    'train/epoch':epoch,
                    'train/loss':running_loss / (batch + 1),
                    'train/f1_score':f1_score(y_true=gt, y_pred=preds, labels=label_list, average='macro', zero_division=0),
                    'train/acc_percent':(correct / total) * 100
                })
            pbar.close()

            _, test_f1, test_acc = self.test(
                model=self.model, test_dataloader=val_dataloader,epoch=epoch,trial=trial.number
            )
    ...
    @torch.no_grad()
    def test(
        self, model: nn.Module, test_dataloader: DataLoader,epoch:int=0,trial=0
